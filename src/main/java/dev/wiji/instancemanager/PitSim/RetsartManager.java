...
public class RetsartManager {
	...
	static {
		((ProxyRunnable) () -> {
			for(PitSimServer activeServer : PitSimServerManager.activeServers) {
				if(activeServer.getStartTime() + RESTART_TIME < System.currentTimeMillis()) {


					for(PitSimServer server : PitSimServerManager.activeServers) {
						if(Math.abs(server.getStartTime() - activeServer.getStartTime()) < RESTART_BUFFER) {
	...
	public static void restartProxy() {
		...
		((ProxyRunnable) () -> {
			...
			if(minutes.get() == 0) {
				...
				for(PitSimServer activeServer : PitSimServerManager.activeServers) {
					ConfigManager.configuration.set(activeServer.getPteroID(), activeServer.getStartTime());
				}
